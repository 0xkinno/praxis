...
def score_provenance(entity: dict) -> DimensionScore:
    ...
    if fields:
        ...
        if ratio == 0:
            score -= 30.0
            evidence.append(f"0/{len(fields)} columns documented (-30)")
        elif ratio < 0.5:
            score -= 15.0
        ...
        score -= 30.0
        evidence.append("No schema metadata available (-30)")
    
    # Glossary terms (0-5 points)
    glossary_terms = entity.get("glossaryTerms") or {}
    terms = glossary_terms.get("terms") or []
    if not terms:
        score -= 5.0
        evidence.append("No glossary terms linked (-5)")
    else:
        evidence.append(f"{len(terms)} glossary terms linked")
    
    # Domain assignment (0-5 points)
    ...
    if not domain_name:
        score -= 5.0
        evidence.append("Not assigned to any domain (-5)")
    else:
